...
class RestrictedResourcesConfig 
{
    ...
    public function __construct(array $value = [])
    {
        if (array_key_exists('operations', $value)) {
            $this->_usedProperties['operations'] = true;
            $this->operations = array_map(fn ($v) => new \Symfony\Config\SyliusApi\FilterEagerLoadingExtension\RestrictedResourcesConfig\OperationsConfig($v), $value['operations']);
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['operations'])) {
            $output['operations'] = array_map(fn ($v) => $v->toArray(), $this->operations);
        }
