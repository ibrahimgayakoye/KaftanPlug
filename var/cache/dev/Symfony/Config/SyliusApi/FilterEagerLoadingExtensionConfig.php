...
class FilterEagerLoadingExtensionConfig 
{
    ...
    public function __construct(array $value = [])
    {
        if (array_key_exists('restricted_resources', $value)) {
            $this->_usedProperties['restrictedResources'] = true;
            $this->restrictedResources = array_map(fn ($v) => new \Symfony\Config\SyliusApi\FilterEagerLoadingExtension\RestrictedResourcesConfig($v), $value['restricted_resources']);
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['restrictedResources'])) {
            $output['restricted_resources'] = array_map(fn ($v) => $v->toArray(), $this->restrictedResources);
        }
