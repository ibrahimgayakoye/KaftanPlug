...
class InMemoryConfig 
{
    ...
    public function __construct(array $value = [])
    {
        if (array_key_exists('files', $value)) {
            $this->_usedProperties['files'] = true;
            $this->files = array_map(fn ($v) => new \Symfony\Config\KnpGaufrette\AdapterConfig\InMemory\FilesConfig($v), $value['files']);
            unset($value['files']);
        }
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['files'])) {
            $output['files'] = array_map(fn ($v) => $v->toArray(), $this->files);
        }
