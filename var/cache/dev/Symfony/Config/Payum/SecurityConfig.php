...
class SecurityConfig 
{
    ...
    public function __construct(array $value = [])
    {
        if (array_key_exists('token_storage', $value)) {
            $this->_usedProperties['tokenStorage'] = true;
            $this->tokenStorage = array_map(fn ($v) => new \Symfony\Config\Payum\Security\TokenStorageConfig($v), $value['token_storage']);
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['tokenStorage'])) {
            $output['token_storage'] = array_map(fn ($v) => $v->toArray(), $this->tokenStorage);
        }
