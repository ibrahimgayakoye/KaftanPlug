...
class DynamicGatewaysConfig 
{
     ...
     * @return $this
     */
    public function sonataAdmin($value): static
    {
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('config_storage', $value)) {
            $this->_usedProperties['configStorage'] = true;
            $this->configStorage = array_map(fn ($v) => new \Symfony\Config\Payum\DynamicGateways\ConfigStorageConfig($v), $value['config_storage']);
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['configStorage'])) {
            $output['config_storage'] = array_map(fn ($v) => $v->toArray(), $this->configStorage);
        }
