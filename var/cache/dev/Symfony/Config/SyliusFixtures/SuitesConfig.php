...
class SuitesConfig 
{
    ...
    public function __construct(array $value = [])
    {
        if (array_key_exists('fixtures', $value)) {
            $this->_usedProperties['fixtures'] = true;
            $this->fixtures = array_map(fn ($v) => new \Symfony\Config\SyliusFixtures\SuitesConfig\FixturesConfig($v), $value['fixtures']);
            unset($value['fixtures']);
        }

        if (array_key_exists('listeners', $value)) {
            $this->_usedProperties['listeners'] = true;
            $this->listeners = array_map(fn ($v) => new \Symfony\Config\SyliusFixtures\SuitesConfig\ListenersConfig($v), $value['listeners']);
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['fixtures'])) {
            $output['fixtures'] = array_map(fn ($v) => $v->toArray(), $this->fixtures);
        }
