...
class CheckoutResolverConfig 
{
    ...
    public function __construct(array $value = [])
    {
        ...
        if (array_key_exists('route_map', $value)) {
            $this->_usedProperties['routeMap'] = true;
            $this->routeMap = array_map(fn ($v) => new \Symfony\Config\SyliusShop\CheckoutResolver\RouteMapConfig($v), $value['route_map']);
    ...
    public function toArray(): array
    {
        ...
        if (isset($this->_usedProperties['routeMap'])) {
            $output['route_map'] = array_map(fn ($v) => $v->toArray(), $this->routeMap);
        }
